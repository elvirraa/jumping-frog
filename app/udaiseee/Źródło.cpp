...
// Pauses the game for given milisecond
void delay(int ms)
{
	clock_t start = clock();
	...
	while ((clock() - start) < (ms / 1000000000.0) * sec)
		;
}
	...
	for (int i = 0; i < H; i++)
	{
		for (int j = 0; j < W; j++)
		{
			...
			else if (i > 0 && i <= NC)
			{
				...
				{
					if (l == 1 && (i == 7 || i == 5 || i == 3))
						continue; // Does not print these cars so that it is not so hard
					if (i == k + 1 && j == c[k].X)
					{
						...
						if (!((i == 3 && (j == 5 || j == 6 || j == 7)) || (i == 4 && (j == 18 || j == 19 || j == 20))))
						{
							...
							if (i == 1 || i == 3)
								_cprintf("\033[33mX\033[0m");
							else if (i == 4 || i == 6)
							{
								if (p[0].moveActive)
									_cprintf("\033[96mX\033[0m");
								else
									_cprintf("X");
							}
							else
								_cprintf("\033[%dmX\033[0m", cC);
							tempBoard[i][j] = 'X';
							isC = true;
							break;
						}
...
void uCr(int frame, Car* cars, Frog* player)
{
	for (int i = 0; i < NC; i++)
	{
		...
		if (frame % cars[i].Speed == 0)
		{
			...
			{
				if (i == 1 || i == 4)
				{ // If the cars are specific then they appear randomly somewhere else
					do
					{
						cars[i].X = rand() % (W - 2) + 1;
...
{ // If they meet
	if (player[0].X == bird[0].X && player[0].Y == bird[0].Y)
		return true; // The stork ate the frog
...
void showScore(int ps)
{
	...
	if (ps > h)
	{
		...
		_cprintf("Sukces wynik to: %d, rekord: %d\n", ps, h);
}
...
int main()
{
	...
	if (l == 0)
	{
		...
		return 0; // Checks if there is level for given number
